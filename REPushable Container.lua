...
local function Climb(data)
    if string.find(types.Container.record(self).mwscript,"climbable")~=nil and self.position.z+10>=data.position.z then
        core.sendGlobalEvent('Teleport', {actor=data, position=util.vector3((data.position.x+self.position.x)/2,(data.position.y+self.position.y)/2,nearby.castRay(self.position,util.transform.move(0,0,200)*self.position).hitPos.z+10),rotation=nil})
    end
end
